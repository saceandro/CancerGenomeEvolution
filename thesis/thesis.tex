...
The observed VAF is interwined with the normal cell contamination, copy number alterations (CNAs), and the abundance ratio of each subtypes.

One of the first computational method which tackle the first problem mentioned above is ABSOLUTE~\cite{carter2012absolute}, which enabled the estimation of the tumor purity and the ploidy avaraged over all the subtypes.
However, it does not attempt subtype decomposition.
...
.
Subtype abundance ratio estimation using the single nucleotide variations (SNVs) was conducted by SciClone~\cite{miller2014sciclone} and PyClone~\cite{roth2014pyclone}.
They utilize Bayesian mixture modeling to cluster mutations which have similar VAF to infer the abundance ratio of each subtypes.
However, neither of them infers the phylogenetic relationship between subtypes.
After that, LICHeE~\cite{popic2015fast} and AncesTree~\cite{el2015reconstruction} enabled to reconstruct phylogenetic relationship as an acyclic directed graph using multiple samples from the same tumor.
Infinite-sites assumption, which assumes that no genomic position, or locus, mutates more than once in the course of clonal evolution, can be used to resolve the ambiguity of the phylogenetic relationship, because the mutations harbored by smaller number of cancer cells cannot be ancestral to the mutations harbored by larger number of cancer cells.
...
The subtype reconstruction from bulk sequencing reads has been progressed in this way, however, none of these methods is able to infer the birth time and growth rate of each subtype, which are the important subtype characteristics in the latest cancer treatment.
This is because they discard passenger mutations, which do not affect the fitness of the subtype and obey neutral evolution.
...
If there exists a single subtype within a tumor, we can infer the birth time using linear regression~\cite{williams2016identification}.
However, if there were multiple subtypes, we cannot infer it by the linear regression.
Here I provide a Bayesian mixture modeling to estimate the birth time of each subtype to estimate birth time and growth rate of each subtype. Our method models the allele frequency drift in each subtype with diffusion equation applying Wright-Fisher process, enabling the inference of the birth time and growth rate of each subtype.

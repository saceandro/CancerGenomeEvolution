...
Then, the subtype $i$'s population at time $t$, $Nn_i(t)$, can be calculated as follows.
\begin{equation}
 n_i(t) = \frac{1}{N}\napier^{\alpha_i (t_i - t)},
\end{equation}
...
\begin{equation}
n_i \coloneqq n_i(0) = \frac{1}{N}\napier^{\alpha_i t_i}.\label{n_i}
\end{equation}

\begin{figure}[H]
\begin{center}
  \includegraphics[bb=9 8 626 373,width=0.4\hsize]{../../dc1/figures/cancer_evolution_one_shrink_jp.pdf}
...
This is a special case of the hypergeometric equation \eqref{hypgeo_concrete}.
% \begin{equation}
...
\begin{align}
 p(x_i | t_i, t_{\mathrm{child}(i)}, n_i) = \begin{cases}
                      \int_{0}^{t_i} f\left(x_i\Big|\frac{1}{Nn_i(t')}, \tau(t')\right) dt' \bigg/ ( t_i - t_{\mathrm{child}(i)} ) & \left( 0 < x_i < 1\right)\\
...
How each passenger mutation is inherited to descendant subtypes is explained in \figref{fig: mutation inheritance}.
I generated the reads using $H=1000, N=10^6, r_i = 1.67\times 10^{-7}$ with varying $B$.\\


 \begin{algorithm}[H]
  \caption{NGS read count simulation}
  ...
  $t = h\varDelta t$
      ...
      \For{$\text{Subtype } i \in \{i | t_i > t \}$}{
          $\lambda_i = Nn_i(t) \times B \times r_i\varDelta t$\\
          $s_i \sim Poisson(s_i | \lambda_i)$

              \For{\text{SNVs } $k_i \in \{1, \cdots, s_i\}$}{
                  $x_{i} \sim f(x_{i} | 1/Nn_i(t), t)$

                  \For{\text{Subtype } $j \in \text{child}(i)$}{
                      \If{$t_j < t$}{
                          $x_{j} \sim Bernoulli(x_{j} | 1/Nn_i(t))$

                          \If{$x_{j} = 1$}{
                             mark\_inherited($child(j)$)
                             }
                         }
                   }
                 $\mu_{k_i} = \sum_{i=1}^{I} n_i x_{i} / 2$
...
\begin{figure}[H]
\begin{center}
   \includegraphics[bb=39 1 525 361,width=0.5\hsize]{master_figures/cancer_evolution_inherit20161215.pdf}
 ...
 The yellow line represents the lineage of the subtype $j$'s founder cell.
...
\begin{align}
 p (m_k | M_k, n, x) = Binom(m_k | M_k, \mu_k) = { m_k \choose M_k } \mu_k^{m_k} \left( 1 - \mu_k \right)^{M_k - m_k}.\label{param_estimation_binomial}
\end{align}
...
$p(h_k | i_k)$ represents the probability that SNV $k$ is inherited to the child subtypes in the pattern $h_k \in \{(0,\cdots,0), (0,\cdots,1), \cdots, (1,\cdots,1)\}$, where the $j$ th element of $h_k$ represents whether the $j$ th child of subtype $i$ inherits the numation or not.
...
which corresponds to equation \eqref{vaf_dist_norm} or \eqref{vaf_dist_norm_inherited} depending on whether SNV $k$ is an isolated mutation (i.e. $h_k = (0,\cdots,0)$) or not.
...
For each SVN locus $k$, $i_k$, $h_k$, $x_{i_k}$, and $q_{k 0:(M_k - 1)} = ( q_{k 0}, \cdots, q_{k (M_k-1)} )$ are assumed to be hidden states because they cannot be observed.
...
& \gamma\left(z^{(k)}_{i_k, h_k, x_{i_k}, q_{k 0:(M_k - 1)}}\right)\\
 ...
 & = \frac{p(i_k) p(h_k | i_k) p(x_{i_k} | i_k, h_k, n_{i_k}^{\mathrm{old}}, t_{i_k}^{\mathrm{old}}, t_{\mathrm{child}(i_k)}^{\mathrm{old}}) \prod_{l=0}^{m_k - 1} \left( n_{q_{kl}}^{\mathrm{old}} \frac{x_{q_{kl}}}{2} \right) \prod_{l=m_k}^{M_k - 1} \left( n_{q_{kl}}^{\mathrm{old}} \left(1-\frac{x_{q_{kl}}}{2} \right) \right) }
...
\vspace{2ex}
Minimization of $Q(t,n; t^{(iter-1)},n^{(iter-1)})$ in each M step is conducted using the L-BFGS-B gradient descent algorithm~\cite{zhu1997algorithm}.

...
How passenger mutation is inherited to its descendants is explained in \figref{fig: mutation inheritance}.
      ...
      \For{$\text{Subtype } i \in \{ t_i > t \}$}{
          ...
          $s_i \sim Poisson(s_i | \lambda_i)$
              ...
              \For{\text{SNVs } $k_i \in \{1, \cdots, s_i\}$}{
                 ...
                 $\mu_{k_i} = \sum_{i=1}^{I} n_i x_{k_i} / 2$
  
                 $m_{i,k_j} \sim Binom(m_{i,k_j} | M, \mu_{k_i})$

                 Reads.append($(m_{i,k_i}, M)$)
               }
      }
  }
  return Reads

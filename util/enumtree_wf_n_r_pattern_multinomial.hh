...
#include <iostream>
#include "../src/alpha_n_pattern_multinomial/setting.hh"
...
params::params(hyperparams& hpa)
{
  for (int i=0; i<=hpa.MAX_SUBTYPE; ++i)
    pa.push_back(new param (Log(0), Log(1), Log(0), Log(0), VLog (hpa.TOTAL_CN + 1, Log(0)), VVLog (hpa.TOTAL_CN + 1, VLog (hpa.TOTAL_CN + 1, Log(0))))); // set mutation rate to be 1 for all subtype
}

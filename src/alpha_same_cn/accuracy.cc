...
double accuracy_u(params& pa, params& pa_ans, hyperparams& hpa)
{
  Log accuracy = Log(0);
...
void read_u(std::ifstream& f, params& pa, hyperparams& hpa)
{
  double a;
...
int main(int argc, char** argv)
{
  ...
  if (argc != 11)
    {
      cerr << "usage: ./accuracy #subtype #topology #locus (beta) (reads) (u beta outfile) (llik outfile) (llik final outfile) (u_ans infile) (accuracy outfile)" << endl;
      exit(EXIT_FAILURE);
    }
  ...
  accuracy_f << res.size() << "\t" << accuracy_u(pa_best, pa_ans, hpa);

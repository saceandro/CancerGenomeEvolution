  ...
  for (int i=-step; i<=step; ++i)
    {
      ...
      gsl_vector_set(x, 2, 1.0 * ((double)i) / step);
      double num = calc_dx_u_llik_numeric(res, x, 2, hpa);
      double analytic = calc_dx_u_llik_analytic(res, x, 2, hpa);
      
      if (fabs(num) > 0)
        ff << gsl_vector_get(x, 2) << "\t" << num << "\t" << analytic << "\t" << fabs(num - analytic) << "\t" << calc_rel_err(num, analytic) << endl;
      else
        ff << gsl_vector_get(x, 2) << "\t" << num << "\t" << analytic << "\t" << fabs(num - analytic) << "\t" << -1 << endl;
    }

...
{
  if (i == j)
    return pa[i]->n * (Log(1.0) - pa[i]->n)/ pa[i]->t;
  else
...
{
  // with regard to u, grad_by_param[i]->u is not multiplied by pa[i]->u
  int K;
  K = res.size();
...
int main(int argc, char** argv)
{
  ...
  if (argc != 8)
    {
      cerr << "usage: ./lda_exptimeweight_grad max_subtype total_cn n (reads) (n pi kappa outfile) (llik outfile) (llik final outfile)" << endl;
      exit(EXIT_FAILURE);
    }

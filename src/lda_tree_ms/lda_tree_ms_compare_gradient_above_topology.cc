...
int main(int argc, char** argv)
{
  ...
  for (int i=-step; i<=step; ++i)
    {
      ...
      double num = calc_dx_u_llik_numeric(res, x, 1, hpa, trs2[a]);
      double analytic = calc_dx_u_llik_analytic(res, x, 1, hpa, trs2[a]);
  ...
  for (int i=-step; i<=step; ++i)
    {
      ...
      double num = calc_dx_pi_llik_numeric(res, x, 1, 1, hpa, trs2[a]);
      double analytic = calc_dx_pi_llik_analytic(res, x, 1, 1, hpa, trs2[a]);
  ...
  for (int i=-step; i<=step; ++i)
    {
      ...
      double num = calc_dx_kappa_llik_numeric(res, x, hpa.MAX_SUBTYPE, hpa.TOTAL_CN, hpa.TOTAL_CN, hpa, trs2[a]);
      double analytic = calc_dx_kappa_llik_analytic(res, x, hpa.MAX_SUBTYPE, hpa.TOTAL_CN, hpa.TOTAL_CN, hpa, trs2[a]);
  ...
  //     double num = calc_dx_kappa_llik_numeric(res, x, 1, 2, 1, hpa, trs2[a]);
  //     double analytic = calc_dx_kappa_llik_analytic(res, x, 1, 2, 1, hpa, trs2[a]);

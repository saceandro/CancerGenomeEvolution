...
#define pat_prob(mut_ratio, start, end) ((mut_ratio) * ((start) - (end)))

Log d_pat_t(subtypes& tr)
{
  int child_num = tr.children.size();
...
void calc_llik_sub(READS& res, QS& qs, params& pa, hyperparams& hpa, subtypes& tr, VVLog& gegen, VLog& gegen_int, VVVLog& vf)
{
}
  ...
  for (int k=0; k<K; ++k)
    {
      ...
      INHERITEDS inh = INHERITEDS (hpa.MAX_SUBTYPE + 1, 0);

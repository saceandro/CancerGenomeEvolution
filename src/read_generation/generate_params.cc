...
int main(int argc, char** argv)
{
  ...
  if (argc != 7)
    {
      cerr << "usage: ./generate_params max_subtype topology seed (purity outfile) (u_n_xi outfile) (t_n outfile)" << endl;
      exit(EXIT_FAILURE);
    }
  ...
  generate_params(pa, hpa, tr[topology], r);

  purity_out << (Log(1) - tr[topology][0].n).eval() << endl;
  write_params(f, pa, hpa, tr[topology]);
  write_t_n(g, tr[topology], hpa);

...
Log variant_fraction_t(int s, Log init_frac, Log n_q, Log t_q, Log t, Log beta_tilda_q, VVLog& gegen)
{
  Log x_q = Log(((double) s) / FRACTIONS);
  double w = 1.0 - 2.0*init_frac.eval();
  
  Log Nnq = Log(CELL_MAX) * n_q;
  Log lNnq = Nnq.take_log_Log();

  Log beki = (t / t_q * lNnq).take_exp();
  ...
  if (0 < s && s < FRACTIONS)
      ...
      for (int i=GEGEN_MAX; i>0; --i)
        {
          ...
          acc +=
            Log((2.0*i + 1.0) / i / (i+1.0)) * Log(gsl_sf_gegenpoly_n(i-1, 1.5, w)) * gegen[s][i] * ( -g * ( beki - Log(1.0) )).take_exp();
        }
      ret = Log(4) * init_frac * (Log(1) - init_frac) * acc;
    }
  ...
  else if (s == FRACTIONS)
    {
      ...
      for (int i=GEGEN_MAX; i>0; --i)
        {
          Log g = calc_gamma_i(i, n_q, t_q, beta_tilda_q);
          Log a =
            Log((2.0*i + 1.0) / i / (i+1.0)) * Log(gsl_sf_gegenpoly_n(i-1, 1.5, w)) * ( -g * ( beki - Log(1.0) )).take_exp();
      ...
      ret = init_frac + Log(2) * init_frac * (Log(1) - init_frac) * acc;
    }
  ...
  else // s == 0
    {
      ...
      ret = Log(1) - init_frac - Log(2) * init_frac * (Log(1) - init_frac) * acc;
    }

  if (ret < Log(0))
    {
      cerr << "err: variant_fraction_t < 0: " << ret.eval() << endl;
      ret = Log(0);
    }
  else if (ret > Log(1))
    {
      cerr << "err: variant_fraction_t > 1: " << ret.eval() << endl;
      ret = Log(1);
    }
  return ret;
}

void variant_fraction_t_partition(Log init_frac, Log n_q, Log t_q, Log t, Log beta_tilda_q, VVLog& gegen, VLog& vf)
{
  Log partition = Log(0);
  ...
  for (int s=0; s<=FRACTIONS; ++s)
    {
      ...
      vf[s] = variant_fraction_t(s, init_frac, n_q, t_q, t, beta_tilda_q, gegen);
...
//   if (argc != 6)
//     {
//       cerr << "usage: ./variant_fraction (init_frac) (n_q) (t_q) (t) (beta_tilda_q)" << endl;
//       return 0;
//     }
...
//   Log init_frac = Log(atof(argv[1]));
//   Log n_q = Log(atof(argv[2]));
//   Log t_q = Log(atof(argv[3]));
//   Log t = Log(atof(argv[4]));
//   Log beta_tilda_q = Log(atof(argv[5]));


//   Log sum = Log(0);
//   for (int s=0; s<=FRACTIONS; ++s)
//     {
//       Log a = variant_fraction_t(s, init_frac, n_q, t_q, t, beta_tilda_q, gegen).eval();
//       cout << a.eval() << "\t";
//     }
//   cout << endl;
  
//   VLog vf (FRACTIONS+1, Log(0));
//   variant_fraction_t_partition(init_frac, n_q, t_q, t, beta_tilda_q, gegen, vf);
  
//   write_vector((ofstream&)cout, vf, FRACTIONS + 1);

...
void calc_params(const gsl_vector* x, params& pa, hyperparams& hpa)
{
  ...
  for (int i=1; i<=hpa.MAX_SUBTYPE; ++i)
    {
      pa.pa[i]->n = Log(gsl_vector_get(x, hpa.MAX_SUBTYPE + i-1));
      cout << pa.pa[i]->n.eval() << "\t";
    }
  cout << endl;
}
...
{
  if (i == j)
    return (Log(1) - pa.pa[0]->n) * pa.pa[i]->n * (Log(1.0) - pa.pa[i]->n);

  return -(Log(1) - pa.pa[0]->n) * pa.pa[i]->n * pa.pa[j]->n;
}

...
typedef struct _ds
{
  ...
  _ds (READS& _res, gsl_vector* _x, int _i, hyperparams& _hpa) : res(_res), x(_x), i(_i), hpa(_hpa) {}
}
  ds;
...
void calc_params(const gsl_vector* x, params& pa, hyperparams& hpa)
{
  for (int i=0; i<hpa.MAX_SUBTYPE; ++i)
    {
      pa[i]->s = Log(calc_sigmoid(gsl_vector_get(x, i)));
      // pa[i]->s = Log(gsl_vector_get(x, i));
    }
  ...
  for (int i=1; i<=hpa.MAX_SUBTYPE; ++i)
    {
      double m = gsl_vector_get(x, hpa.MAX_SUBTYPE + params_per_subtype * (i-1));
      
      for (int l=2; l<=hpa.TOTAL_CN; ++l)
        {
          double s = gsl_vector_get(x, hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + l-1);
          if (m < s) m = s;
        }
      ...
      for (int l=1; l<=hpa.TOTAL_CN; ++l)
        {
          pa[i]->pi[l] = Log(gsl_vector_get(x, hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + l-1) - m, 1);
          sum += pa[i]->pi[l];
        }
      ...
      for (int l=1; l<=hpa.TOTAL_CN; ++l)
        {
          m = gsl_vector_get(x, hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + hpa.TOTAL_CN + (l-1) * l / 2);
          for (int r=2; r<=l; ++r)
            {
              double s = gsl_vector_get(x, hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + hpa.TOTAL_CN + (l-1) * l / 2 + r-1);
              if (m < s) m = s;
            }
          ...
          for (int r=1; r<=l; ++r)
            {
              pa[i]->kappa[l][r] = Log(gsl_vector_get(x, hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + hpa.TOTAL_CN + (l-1) * l / 2 + r-1) - m, 1);
              sum += pa[i]->kappa[l][r];
            }
...
double calc_llik_for_ds(double x_i, void* _ds)
{
  ds* p = (ds*) _ds;
...
double calc_llik_for_dpi(double x_il, void* dp)
{
  ...
  gsl_vector_set(p->x, p->hpa.MAX_SUBTYPE + params_per_subtype * (p->i - 1) + p->l - 1, x_il);
...
double calc_llik_for_dkappa(double x_ilr, void* dk)
{
  ...
  gsl_vector_set(p->x, p->hpa.MAX_SUBTYPE + params_per_subtype * (p->i - 1) + p->hpa.TOTAL_CN + (p->l - 1) * p->l / 2 + p->r - 1, x_ilr);
...
double d_llik(READS& res, params& pa, params& grad_by_param, hyperparams& hpa)
{
  ...
  for (int i=0; i<hpa.MAX_SUBTYPE; ++i)
    grad_by_param[i]->s += Log(hpa.be_hpa.first - 1.0) / pa[i]->s - Log(hpa.be_hpa.second - 1.0) / (Log(1.0) - pa[i]->s);

  for (int i=1; i<=hpa.MAX_SUBTYPE; ++i)
    {
      for (int l=1; l<=hpa.TOTAL_CN; ++l)
        {
          grad_by_param[i]->pi[l] += Log(hpa.alpha[l] - 1.0);

          for (int r=1; r<=l; ++r)
            {
              grad_by_param[i]->kappa[l][r] += Log(hpa.beta[l][r] - 1.0);
            }
        }
    }
...
double calc_dx_s_llik_numeric(READS& res, gsl_vector* x, int i, hyperparams& hpa)
{
  gsl_function F;
  ds _ds (res, x, i, hpa);
  _ds.res = res;
...
double calc_dx_s_llik_analytic(READS& res, gsl_vector* x, int j, hyperparams& hpa)
{
  int K = res.size();
  ...
  // return calc_dx_sigmoid(gsl_vector_get(x,j)) * grad_by_param[j]->s.eval();
  return (Log(calc_dx_sigmoid(gsl_vector_get(x,j))) * grad_by_param[j]->s).eval();
  // return (Log(calc_dx_sigmoid(pa[j]->u.eval())) * grad_by_param[j]->s).eval();
  // return grad_by_param[j]->s.eval();
}
...
double calc_dx_pi_llik_numeric(READS& res, gsl_vector* x, int i, int l, hyperparams& hpa)
{
  ...
  double result, abserr;
  int params_per_subtype = hpa.TOTAL_CN * (hpa.TOTAL_CN + 3) / 2;
  gsl_deriv_central(&F, gsl_vector_get(x, hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + l-1), 1e-5, &result, &abserr);
...
double calc_dx_kappa_llik_numeric(READS& res, gsl_vector* x, int i, int l, int r, hyperparams& hpa)
{
  ...
  double result, abserr;
  int params_per_subtype = hpa.TOTAL_CN * (hpa.TOTAL_CN + 3) / 2;
  gsl_deriv_central(&F, gsl_vector_get(x, hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + hpa.TOTAL_CN + (l-1) * l / 2 + r-1), 1e-5, &result, &abserr);
...
void gsl_set_random(gsl_vector* x, hyperparams& hpa, gsl_rng* rng)
{
  ...
  for (int i = 0; i<hpa.MAX_SUBTYPE + hpa.MAX_SUBTYPE * hpa.TOTAL_CN * (hpa.TOTAL_CN + 3) / 2; ++i)
    {
...
int main(int argc, char** argv)
{
  ...
  gsl_vector* x = gsl_vector_alloc(hpa.MAX_SUBTYPE + hpa.MAX_SUBTYPE * hpa.TOTAL_CN * (hpa.TOTAL_CN + 3) / 2);
  ...
  for (int i=-step; i<=step; ++i)
    {
      ...
      double num = calc_dx_s_llik_numeric(res, x, 0, hpa);
      double analytic = calc_dx_s_llik_analytic(res, x, 0, hpa);
  ...
  for (int i=-step; i<=step; ++i)
    {
      gsl_set_random(x, hpa, r);
      gsl_vector_set(x, hpa.MAX_SUBTYPE, 1.0 * ((double)i) / step);
      double num = calc_dx_pi_llik_numeric(res, x, 1, 1, hpa);
      double analytic = calc_dx_pi_llik_analytic(res, x, 1, 1, hpa);
      
      if (fabs(num) > 0)
        g << gsl_vector_get(x, hpa.MAX_SUBTYPE) << "\t" << num << "\t" << analytic << "\t" << fabs(num - analytic) << "\t" << calc_rel_err(num, analytic) << endl;
      else
        g << gsl_vector_get(x, hpa.MAX_SUBTYPE) << "\t" << num << "\t" << analytic << "\t" << fabs(num - analytic) << "\t" << -1 << endl;
    }

  for (int i=-step; i<=step; ++i)
    {
      gsl_set_random(x, hpa, r);
      gsl_vector_set(x, hpa.MAX_SUBTYPE + hpa.MAX_SUBTYPE * hpa.TOTAL_CN * (hpa.TOTAL_CN + 3) / 2 - 1, 1.0 * ((double)i) / step);
      ...
      if (fabs(num) > 0)
        h << gsl_vector_get(x, hpa.MAX_SUBTYPE + hpa.MAX_SUBTYPE * hpa.TOTAL_CN * (hpa.TOTAL_CN + 3) / 2 - 1) << "\t" << num << "\t" << analytic << "\t" << fabs(num - analytic) << "\t" << calc_rel_err(num, analytic) << endl;
      else
        h << gsl_vector_get(x, hpa.MAX_SUBTYPE + hpa.MAX_SUBTYPE * hpa.TOTAL_CN * (hpa.TOTAL_CN + 3) / 2 - 1) << "\t" << num << "\t" << analytic << "\t" << fabs(num - analytic) << "\t" << -1 << endl;
  }

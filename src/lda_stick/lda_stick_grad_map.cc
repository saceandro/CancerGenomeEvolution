...
void my_df (const gsl_vector *v, void *par, gsl_vector *df)
{
  ...
  for (int j=0; j<p->hpa.MAX_SUBTYPE; ++j)
    {
      double grad = (Log(calc_dx_sigmoid(gsl_vector_get(v,j))) * grad_by_param[j]->s).eval();
      ...
      for (int s=1; s<=p->hpa.TOTAL_CN; ++s)
        {
          ...
          gsl_vector_set(df, p->hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + s-1, -grad);
        }
          ...
          for (int t=1; t<=l; ++t)
            {
              ...
              gsl_vector_set(df, p->hpa.MAX_SUBTYPE + params_per_subtype * (i-1) + p->hpa.TOTAL_CN + (l-1) * l / 2 + t-1, -grad);
            }
        }
    }
}
...
double minimize(diff& di, params& pa, ofstream& h, gsl_rng* rng)
{
  ...
  my_func.n = di.hpa.MAX_SUBTYPE + di.hpa.TOTAL_CN * (di.hpa.TOTAL_CN + 3) / 2 * di.hpa.MAX_SUBTYPE;
  my_func.f = my_f;
  my_func.df = my_df;
  my_func.fdf = my_fdf;
  my_func.params = &di;
...
int main(int argc, char** argv)
{
  ...
  if (argc != 8)
    {
      cerr << "usage: ./lda_stick_grad_map max_subtype total_cn n (reads) (n pi kappa outfile) (llik outfile) (llik final outfile)" << endl;
      exit(EXIT_FAILURE);
    }

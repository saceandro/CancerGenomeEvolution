...
public:
  ...
  diff (READS& _res, hyperparams& _hpa, subtype& _st, VVLog& _gegen, VLog& _gegen_int) : res(_res), hpa(_hpa), st(_st), gegen(_gegen), gegen_int(_gegen_int) {}
};
...
double my_f (const gsl_vector *v, void *par)
{
  ...
  double llik = calc_llik(p->res, pa, p->hpa, p->st, p->gegen, p->gegen_int);
...
void my_df (const gsl_vector *v, void *par, gsl_vector *df)
{
  ...
  double llik = d_llik(p->res, pa, grad, p->hpa, p->st, p->gegen, p->gegen_int, d_t_variant_fraction);

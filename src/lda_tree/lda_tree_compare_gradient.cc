...
double calc_mu(subtypes& st, params& pa, hyperparams& hpa)
{
  ...
  for (int i=0; i<=hpa.MAX_SUBTYPE; ++i)
    {
      denom += pa[i]->n * Log(st[i].total_cn);
      num += pa[i]->n * Log(st[i].variant_cn);
    }
...
Log d_mu_n(subtypes& st, params& pa, hyperparams& hpa, int j)
{
  Log normal;
  for (int i=0; i<=hpa.MAX_SUBTYPE; ++i)
    normal += pa[i]->n * Log(st[i].total_cn);

  Log variant;
  for (int i=0; i<=hpa.MAX_SUBTYPE; ++i)
    variant += pa[i]->n * Log(st[i].variant_cn);
...
void responsibility_numerator(READ& re, states& sts, subtypes& st, params& pa, hyperparams& hpa)
{
  ...
  for (int i=1; i<=hpa.MAX_SUBTYPE; ++i)
    product *= pa[i]->pi[st[i].total_cn] * pa[i]->kappa[st[i].total_cn][st.st[i].variant_cn];
...
//     return 0;
// }
void calc_params(const gsl_vector* x, params& pa, hyperparams& hpa, subytpes& st)
{
  for (int i=0; i<=hpa.MAX_SUBTYPE; ++i)
    {

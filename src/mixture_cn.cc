...
double lik_sub(read& re, state& st, Vuint& cns, Vdouble& kappa, int& curr_subtype)
{
  ...
  if (st->nxt)
    {
      for (int st->variant_cn = 1; st->varint_cn <= TOTAL_CN; ++st->varint_cn)
        {
          cns[curr_subtype] = st->variant_cn;
          sum += kappa[st->variant_cn] * lik_sub(re, st->nxt, cns, kappa, curr_subtype + 1);
        }
    }
  ...
  else
    {
      for (int st->variant_cn = 1; st->varint_cn <= TOTAL_CN; ++st->varint_cn)
        {
          cns[curr_subtype] = st->variant_cn;
          double mu = calc_mu(cns);
          sum += kappa[st->variant_cn] * gsl_ran_binomial_pdf(re.first, mu, re.second);
        }
    }
  return sum;
}
  ...
  if (st->nxt)
    {
      for (int st->variant_cn = 1; st->varint_cn <= TOTAL_CN; ++st->varint_cn)
        {
          cns[curr_subtype] = st->variant_cn;
          sum += d_kappa_llik_sub(re, st->nxt, cns, kappa, curr_subtype + 1);
        }
    }

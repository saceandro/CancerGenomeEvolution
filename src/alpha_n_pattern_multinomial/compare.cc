...
{
  for (int i=st.children.size()-1; i>=0; ++i)
    {
      if ((pat >> i) % 2 == 1)
        {
      ...
      for (int pat=0; pat<pow(2,tr[index].children.size()); ++pat)
        {
          ...
          lik_k += lik_k_pat / Log(tr[index].children.size() + 1);
        }
      lik *= lik_k;
    }
      ...
      for (int pat=0; pat<pow(2,tr[index].children.size()); ++pat)
        {
          ...
          for (; s<=FRACTIONS; ++s)
            {
              ...
              d_th_k_pat += Log(log_binomial_pdf(res[k]->first, mu, res[k]->second), 1) * dthvf[index][eldest_ch_index][s];
          ...
          lik_k += lik_k_pat / Log(tr[index].children.size() + 1);
          d_t_k += d_t_k_pat / Log(tr[index].children.size() + 1);
          if (pat != 0)
            d_th_k += d_th_k_pat / Log(tr[index].children.size() + 1);
          for (int i=1; i<=hpa.MAX_SUBTYPE; ++i) // start from subtype 1
            d_n_k[i] += d_n_k_pat[index] / Log(tr[index].children.size() + 1);
        }

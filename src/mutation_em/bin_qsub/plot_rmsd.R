...
## par(cex.lab=3)
## par(cex.axis=1.6)
boxplot(d[,3]~d[,1],col="bisque",xlab="Number of SNVs", ylab="RMSD")
dev.off()

pdf(paste(argv[1], ".t.pdf", sep=""), height=7, width=7)
boxplot(d[,4]~d[,1],col="bisque",xlab="Number of SNVs", ylab="RMSD")
dev.off()

pdf(paste(argv[1], ".n.pdf", sep=""), height=7, width=7)
boxplot(d[,5]~d[,1],col="bisque",xlab="Number of SNVs", ylab="RMSD")
dev.off()

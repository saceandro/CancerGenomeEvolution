...
int calc_fdf(VLog& du, VLog& dn, Log& qfunc, Log& llik, hyperparams& hpa, int num_of_split, string& u1, string& u2, string& n, string& snvs, string& coverage, string& seed, string& est_iter)
{
  double a;
  ...
  for (int sp=0; sp<num_of_split; ++sp)
    {
      ...
      ifstream f (("../du_dn_llik_qsub_coverage_seed_rand_param/" + u1 + "/" + u2 + "/" + n + "/" + snvs + "/" + coverage + "/" + seed + "/" + est_iter + "/" + filenum_str.str()).c_str());
...
struct ComputeFdf {
  ...
  int operator()(const Vdouble& x, double& fn, vector<double>& gr)
  {
    ...
    // for (int sp=0; sp<num_of_split; ++sp)
    //   {
        // std::ostringstream datafile_id;
        // datafile_id << sp;
        system(("./estep_coverage_seed_rand_param_singleprocess.sh " + u1 + " " + u2 + " " + n + " " + snvs + " " + coverage + " " + seed + " " + est_iter + " 0").c_str());
      // }
    ...
    int qfunc_flag = 0;
    qfunc_flag = calc_fdf(du, dn, qfunc, llik_final, hpa, num_of_split, u1, u2, n, snvs, coverage, seed, est_iter);
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 24)
    {
      cerr << "usage: ./mstep_qsub_coverage_seed_rand_param_singleprocess subtype topology num_of_split (x y init infile) (pa_test outfile) (vf_test outfile) (llik outfile) (pa_best outfile) (vf_old outfile) (params log fie) (true params) (rmsd file) (param difference) (u1) (u2) (n) (#SNV) (coverage) (seed) (iter) (u lower bound) (em max iter) (grad desc max iter)" << endl;
      exit(EXIT_FAILURE);
    }
  ...
  string n (argv[16]);
  string snvs (argv[17]);
  string coverage (argv[18]);
  string seed (argv[19]);
  string est_iter (argv[20]);
  double u_lower = atof(argv[21]);
  int em_max_iter = atoi(argv[22]);
  int grad_desc_max_iter = atoi(argv[23]);

  cerr << "snvs: " << snvs << "\t" << "coverage: " << coverage << "\t"  << "seed: " << seed << "\t" << "iter: " << est_iter << endl;;
  ...
  for (int i=1; i<=em_max_iter; ++i) // do not use index i to loop subtype in this block!
    {
      ...
      int flag = 0;
      Log llik_final (0);
      minimizer.minimize(x0, ComputeFdf(trs[topology], hpa, pa_test_file, vf_test_file, gegen, gegen_int, num_of_split, llik_final, u1, u2, n, snvs, coverage, seed, est_iter, flag));

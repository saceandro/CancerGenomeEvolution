...
void generate_params(params& pa, hyperparams& hpa, subtypes& tr, gsl_rng* rng, double n, int larger)
{
  ...
  if (larger == 1)
    w[1] = gsl_ran_flat(rng, n, 1);
  else
    w[1] = gsl_ran_flat(rng, 0, n);
...
int main(int argc, char** argv)
{
  ...
  if (argc != 7)
    {
      cerr << "usage: ./calc_reverse_param subtype topology (x y u n outfile) seed n larger" << endl;
      exit(EXIT_FAILURE);
    }
  ...
  params pa_old(hpa);
  generate_params(pa_old, hpa, trs[topology], r, n, larger);

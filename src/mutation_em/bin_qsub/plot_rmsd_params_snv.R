...
## par(cex.lab=3)
## par(cex.axis=1.6)
boxplot(d[,3]~d[,2],col="bisque",xlab=expression(paste("True ", t[2])), ylab="RMSD")
dev.off()

pdf(paste(argv[1], ".t.pdf", sep=""), height=7, width=7)
boxplot(d[,4]~d[,2],col="bisque",xlab=expression(paste("True ", t[2])), ylab="RMSD")
dev.off()

pdf(paste(argv[1], ".n.pdf", sep=""), height=7, width=7)
boxplot(d[,5]~d[,2],col="bisque",xlab=expression(paste("True ", t[2])), ylab="RMSD")
dev.off()

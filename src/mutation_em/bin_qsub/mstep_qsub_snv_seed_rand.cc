...
int calc_fdf(VLog& du, VLog& dn, Log& qfunc, Log& llik, hyperparams& hpa, int num_of_split, string& snvs, string& seed, string& est_iter)
{
  double a;
  ...
  for (int sp=0; sp<num_of_split; ++sp)
    {
      ...
      ifstream f (("../du_dn_llik_qsub_snv_seed_rand/" + snvs + "/" + seed + "/" + est_iter + "/" + filenum_str.str()).c_str());
...
struct ComputeFdf {
  ...
  int operator()(const Vdouble& x, double& fn, vector<double>& gr)
  {
    ...
    system(("qsub -N estep_snv" + snvs + "_seed" + seed + "_rand" + est_iter + " -e ../log_qsub_snv_seed_rand/estep.err -o ../log_qsub_snv_seed_rand/estep.log -sync y -tc 100 -t 1-" + filenum_str.str() + ":1 estep_snv_seed_rand.sh " + snvs + " " + seed + " " + est_iter).c_str());
    ...
    int qfunc_flag = 0;
    qfunc_flag = calc_fdf(du, dn, qfunc, llik_final, hpa, num_of_split, snvs, seed, est_iter);
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 20)
    {
      cerr << "usage: ./mstep_qsub_snv_seed_rand subtype topology num_of_split (x y init infile) (pa_test outfile) (vf_test outfile) (llik outfile) (pa_best outfile) (vf_old outfile) (params log fie) (true params) (rmsd file) (param difference) (#SNV) (seed) (iter) (u lower bound) (em max iter) (grad desc max iter)" << endl;
      exit(EXIT_FAILURE);
    }
  ...
  for (int i=1; i<=em_max_iter; ++i) // do not use index i to loop subtype in this block!
    {
      ...
      int flag = 0;
      Log llik_final (0);
      minimizer.minimize(x0, ComputeFdf(trs[topology], hpa, pa_test_file, vf_test_file, gegen, gegen_int, num_of_split, llik_final, snvs, seed, est_iter, flag));

...
int calc_fdf(VLog& du, VLog& dn, Log& qfunc, Log& llik, hyperparams& hpa, int num_of_split, string& coverages, string& seed, string& est_iter)
{
  double a;
  ...
  for (int sp=0; sp<num_of_split; ++sp)
    {
      ...
      ifstream f (("../du_dn_llik_qsub_coverage_seed_rand/" + coverages + "/" + seed + "/" + est_iter + "/" + filenum_str.str()).c_str());
...
struct ComputeFdf {
  ...
  int operator()(const Vdouble& x, double& fn, vector<double>& gr)
  {
    ...
    system(("qsub -N estep_coverage" + coverages + "_seed" + seed + "_rand" + est_iter + " -e ../log_qsub_coverage_seed_rand/estep.err -o ../log_qsub_coverage_seed_rand/estep.log -sync y -tc 100 -t 1-" + filenum_str.str() + ":1 estep_coverage_seed_rand.sh " + coverages + " " + seed + " " + est_iter).c_str());
    ...
    int qfunc_flag = 0;
    qfunc_flag = calc_fdf(du, dn, qfunc, llik_final, hpa, num_of_split, coverages, seed, est_iter);
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 20)
    {
      cerr << "usage: ./mstep_qsub_coverage_seed_rand subtype topology num_of_split (x y init infile) (pa_test outfile) (vf_test outfile) (llik outfile) (pa_best outfile) (vf_old outfile) (params log fie) (true params) (rmsd file) (param difference) (#COVERAGE) (seed) (iter) (u lower bound) (em max iter) (grad desc max iter)" << endl;
      exit(EXIT_FAILURE);
    }
  ...
  string seed (argv[15]);
  string est_iter (argv[16]);
  double u_lower = atof(argv[17]);
  int em_max_iter = atoi(argv[18]);
  int grad_desc_max_iter = atoi(argv[19]);

  cerr << "coverages: " << coverages << "\t" << "seed: " << seed << "\t" << "iter: " << est_iter << endl;;
  ...
  for (int i=1; i<=em_max_iter; ++i) // do not use index i to loop subtype in this block!
    {
      ...
      int flag = 0;
      Log llik_final (0);
      minimizer.minimize(x0, ComputeFdf(trs[topology], hpa, pa_test_file, vf_test_file, gegen, gegen_int, num_of_split, llik_final, coverages, seed, est_iter, flag));

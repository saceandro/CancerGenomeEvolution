...
double calc_llik(READS& res, statess& _statess, params& pa_new, hyperparams& hpa, subtypes& tr, VVVLog& vf_new, VVLog& gegen, VLog& gegen_int)
{
  int K;
  K = res.size();
          ...
          for (int i=1; i<=hpa.MAX_SUBTYPE; ++i)
            {
              ...
              dn_st[i] += d_xi_n(tr, v[st->q], v[i], st->q, i, Log(CELL_MAX)) / tr[st->q].xi + Log(count[i]) / tr[i].n;
            }
...
int main(int argc, char** argv)
{
  ...
  if (argc != 10)
    {
      cerr << "usage: ./estep_mem subtype topology n (u n old infile) (u n test infile) (vf_old infile) (vf_test infile) (reads infile) (du dn llik outfile)" << endl;
      exit(EXIT_FAILURE);
    }

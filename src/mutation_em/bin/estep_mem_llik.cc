...
double calc_qfunc(READS& res, params& pa_old, VVVLog& vf_old, params& pa_new, hyperparams& hpa, subtypes& tr_old, subtypes& tr, VVVLog& vf_new)
{
  int K;
  K = res.size();
...
int main(int argc, char** argv)
{
  ...
  if (argc != 10)
    {
      cerr << "usage: ./estep_mem_llik subtype topology n (u n old infile) (u n test infile) (vf_old infile) (vf_test infile) (reads infile) (llik outfile)" << endl;
      exit(EXIT_FAILURE);
    }
  ...
  // Log lik = calc_qfunc(res, pa_old, vf_old, pa_test, hpa, trs_old[topology], trs[topology], vf_test);
  g << lik.eval() << endl;
